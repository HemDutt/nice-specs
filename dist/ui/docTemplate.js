...
function renderDocumentation(metadata, doc) {
...
function renderInventoryTable(entries) {
    ...
    for (const entry of entries) {
        rows.push(`| \`${entry.file}\` | ${escapeTableCell(entry.synopsis || '_Pending_')} | ${escapeTableCell(formatSymbolCell(entry.symbols))} |`);
    }
    return rows;
}
function formatSymbolCell(symbols) {
    if (!symbols.length) {
